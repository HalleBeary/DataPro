...
- Data Anlysis agent (agent/analysis_agent.py): Takes in a NL query via the command line interface, generates SQL command, and retrieves desired data from the relevant library. The agent can access different types of SQL databases. Databases currently are chinook.db, northwind_small.sqlite, and are included in `data/`. Easy integration of additional databases is done via YAML configuration
